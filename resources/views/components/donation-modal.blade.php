                ...
                <form class="form-horizontal" method="POST" action="{{ route('donations.store') }}" id="donation-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="number" name="amount" id="amount" class="form-control" required
                                min="25" placeholder="Montant du don">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="email" class="form-control"
                            placeholder="Email pour le reçu" value="{{ auth()->user()->email ?? '' }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                            ...
                            @foreach ($paymentMethods as $method)
                                <option value="{{ $method->id }}" data-color="{{ $method->color_code }}"
                                    data-regex="{{ $method->phone_regex }}" data-prefix="{{ $method->phone_prefix }}">
                                    {{ $method->display_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" name="phone" id="phone" class="form-control"
                                placeholder="Ex: 6XXXXXXXX" value="{{ auth()->user()->phoneNumber ?? '' }}" required>
                            <div class="input-group-append">
                        ...
                        <small class="form-text text-muted operator-hint">Veuillez sélectionner une méthode de
                            paiement.</small>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="alert alert-info">
                            <i class="fa fa-info-circle"></i> Après avoir soumis ce formulaire, vous recevrez un SMS
                            pour finaliser votre don via le service choisi.
                        </div>
                    </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.valid) {
                            ...
                            $('.operator-hint').removeClass('text-danger').addClass('text-success')
                                .text(response.message);
                        } else {
                            $('#phone').removeClass('is-valid').addClass('is-invalid');
                            $('.operator-hint').removeClass('text-success').addClass('text-danger')
                                .text(response.message);
                        }
                    },
                    error: function() {
                        $('.operator-hint').removeClass('text-success').addClass('text-danger').text(
