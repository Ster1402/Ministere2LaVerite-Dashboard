                    ...
                    <tr id="message-{{ $message->id }}">
                        ...
                        <td>
                            <form method="GET" action="">
                                ...
                                <button type="submit" class="btn btn-outline-info ">{{ $message->category }}</button>
                            </form>
                        </td>
                        <td>{{ $message->updated_at?->diffForHumans() }} {{ ($message->created_at != $message->updated_at) ? __(' - modifié') : ''  }}</td>
                        @if(Auth::user()->is($data->first()?->sender))
                            <td>{{ $message->receiver->name ?? $message->assemblies->reduce(fn($acc, $ass) => $ass->name . ', ' . $acc, '') }}</td>
                        @endif
                        <td>
                            @if($message->deleted)
                                Message supprimé
